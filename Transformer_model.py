...
class TransformerImputer(nn.Module):
            ...
            for epoch in range(epochs):
                ...
                for (batch_x,) in loader:
                    ...
                    with torch.amp.autocast('cuda'):
                        ...
                        mit_loss = torch.abs(predictions[mit_mask] - target[mit_mask]).mean() \
                            if mit_mask.sum() > 0 else torch.tensor(0.0, device=device)
                        observed_mask = original_mask.bool() & ~mit_mask
                        ort_loss = torch.abs(predictions[observed_mask] - target[observed_mask]).mean() \
                            if observed_mask.sum() > 0 else torch.tensor(0.0, device=device)
                ...
                if num_batches > 0:
                    ...
                    current_loss = (val_loss if use_validation_for_early_stopping 
                                    and val_loss and not np.isnan(val_loss) else avg_loss)
                    if current_loss < best_loss - min_delta:
                        best_loss, patience_counter = current_loss, 0
                    else:
                        patience_counter += 1
                    if patience_counter >= patience:
                        print(f"Učenje se je predčasno ustavilo pri epohi {epoch} (najnižja izguba: {best_loss:.6f})")
                        break
            print(f"Učenje zaključeno. Končna vrednost izgube: {best_loss:.6f}")
    ...
    def impute(self, dataset):
        try:
            ...
            print(f"Napaka pri zapolnjevanju manjkajočih vrednosti: {e}")
            return None
